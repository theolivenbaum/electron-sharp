                            ...
                            if (HybridSupport.IsElectronActive)
                            {
                                ...
                                socket.OnConnected += (_, __) =>
                                {
                                    Task.Run(async () =>
                                    {
                                        ...
                                        Log("ElectronSharp socket {1} connected on port {0}!", BridgeSettings.SocketPort, socket.Id);
                                    });
                                };
                                ...
                                {
                                    _connectedSocketEvent.Reset();
                                    Log("ElectronSharp socket {1} is trying to reconnect on port {0}...", BridgeSettings.SocketPort, socket.Id);
                                };

                                socket.OnReconnectError += (_, ex) =>
                                {
                                    _connectedSocketEvent.Reset();
                                    Log("ElectronSharp socket {1} failed to connect {0}", ex, socket.Id);
                                };
                                ...
                                {
                                    _connectedSocketEvent.Reset();
                                    Log("ElectronSharp socket {1} failed to reconnect {0}", ex, socket.Id);
                                };

                                socket.OnReconnected += (_, __) =>
                                {
                                    _connectedSocketEvent.Set();
                                    Log("ElectronSharp socket {1} reconnected on port {0}...", BridgeSettings.SocketPort, socket.Id);
                                };

                                socket.OnDisconnected += (_, reason) =>
                                {
                                    _connectedSocketEvent.Reset();
                                    Log("ElectronSharp socket {2} disconnected with reason {0}, trying to reconnect on port {1}!", reason, BridgeSettings.SocketPort, socket.Id);
                                };

                                socket.OnError += (_, msg) =>
                                {
                                    //_connectedSocketEvent.Reset();
                                    Log("ElectronSharp socket {1} error: {0}...", msg, socket.Id);
                                };
